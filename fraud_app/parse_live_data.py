...
class FraudModel(object):
    ...
    def __init__(self, df):
        self.olddf = pd.read_csv('data/fraud_data.csv')
        self.df = df
...
if __name__ == '__main__':
    with open('data/sample_str') as f:
        f = f.read()
        newdf = json_normalize(json.loads(f))
    fm = FraudModel(newdf)
    newdf = fm.preprocess_data()
    newdf.dropna(inplace=True)
    model = run_model(newdf)
    # graph_feature_importance(fm.olddf, model)
